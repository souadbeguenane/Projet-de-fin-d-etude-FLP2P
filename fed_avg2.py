...
import copy
from matplotlib import get_data_path, transforms
import numpy as np
...
from sklearn.cluster import KMeans  # Import de scikit-learn pour le clustering

from data import MNISTDataset, FederatedSampler, mnist
from models import CNN, MLP
from utils import arg_parser, average_weights, Logger


class FedAvg:
    def __init__(self, args: Dict[str, Any], client_data: List[np.ndarray] = None):
        self.args = args
        ...
        # Store client data if provided
        self.client_data = client_data if client_data is not None else []
    ...
    def cluster_clients(self, nodes, k, R):
    ...
    def train_clusters(self):
        # Create an empty list to store cluster results
        cluster_results = []

        for cluster_idx, cluster in enumerate(self.client_data):
            cluster_args = self.args
            cluster_args.n_clients = len(cluster)

            cluster_fed_avg = FedAvg(cluster_args, client_data=cluster)
            cluster_fed_avg.train()

            # Store the results of this cluster's training
            cluster_results.append({
                "cluster_idx": cluster_idx,
                "model_state_dict": cluster_fed_avg.root_model.state_dict(),
                "train_losses": cluster_fed_avg.train_losses,
            })
...
if __name__ == "__main__":
    args = arg_parser()
    client_data = []  # You should populate this list with your client data.

    fed_avg = FedAvg(args, client_data=client_data)
    cluster_results = fed_avg.train_clusters()
